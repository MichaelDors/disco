...
function render404(container) {
  ...
  container.innerHTML = `
    <div style="text-align: center; margin-top: 100px;">
      <h2>404 - Not Found</h2>
      <a href="/" style="color: var(--text-main); text-decoration: underline;">Return Home</a>
    </div>
  `;
}
  ...
  container.innerHTML = `
    ...
    <section class="release-section">
      <div class="release-grid">
        ${artists.map(artist => `
          <a class="release-card" href="/${artist.slug}" onclick="navigate(event, '/${artist.slug}')">
...
function renderArtistPage(container, artist, releases) {
  ...
  const releasesHtml = releases?.map(release => `
    <a class="release-card" href="/${artist.slug}/${release.slug}" onclick="navigate(event, '/${artist.slug}/${release.slug}')">
    ...
    <header id="stickyHeader" class="sticky-header" aria-hidden="true" onclick="window.scrollTo({top: 0, behavior: 'smooth'})">
      <div class="sticky-header-content">
        ...
        <div class="sticky-meta">
          ...
          <a class="sticky-artist" href="/${artist.slug}" onclick="navigate(event, '/${artist.slug}')">${artist.name}</a>
        </div>
  ...
  container.innerHTML = `
    ...
    <section class="hero">
      ...
      <div class="meta">
        ...
        <a class="artist" id="artistName" href="/${artist.slug}" onclick="navigate(event, '/${artist.slug}')">${artist.name}</a>
